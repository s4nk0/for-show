        ...
        <div class="modal-content callback">
            <div class="modal-header callback__header">
                <div class="m-auto d-flex justify-content-center flex-column align-items-center">
                    ...
                    <h4 class="text-center mt-3">{{__('messages.callback.call_me.title')}}</h4>
                    <p class="text-center">{{__('messages.callback.call_me.text')}}</p>
                </div>
            </div>
            <form action="#" class="modal-body callback__form p-0" method="POST">
                @csrf

                <input type="hidden" name="product_id" value="{{ Route::is('product') && isset($product) ? $product->id : null }}">

                <div class="callback__content row m-0 px-4 bg-light">
                    <div class="col-12 col-md-6 mb-3">
                        <input type="hidden" name="type" @isset($type) value="{{$type}}" @else value={{\App\Models\Callback::MAIN}} @endisset>
                        <div class="md-form mb-0 md-outline input-with-pre-icon">
                            ...
                            <label for="callback-name">{{__('messages.your_name')}}</label>
                        </div>
                    </div>
                ...
                <div class="modal-footer p-3">
                    <div class="text-right">
                        <button type="button" data-dismiss="modal" class="button btn-light rounded mr-3">{{__('messages.close')}}</button>
                        <button type="submit" class="button btn-primary rounded">{{__('messages.send')}}</button>
                    </div>
                </div>
                <div class="mb-3">
            ...
            <div class="application__success" style="display: none;">
                <div class="d-flex justify-content-center flex-column align-items-center py-5">
                    ...
                    <h5 class="text-center mt-4 w-65">{{__('messages.callback.success')}}</h5>
                </div>
            </div>
            <div class="application__error" style="display: none;">
                <div class="d-flex justify-content-center flex-column align-items-center py-5">
                    ...
                    <h5 class="text-center mt-4 w-65">{{__('messages.callback.error')}}</h5>
                </div>
            </div>
        </div>
    </div>
