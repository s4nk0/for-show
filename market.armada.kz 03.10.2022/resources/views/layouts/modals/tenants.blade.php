        ...
        <div class="modal-dialog modal-dialog-centered modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header px-md-4">
                    <h5 class="modal-title">{{__('messages.application.for_rent')}}</h5>
                ...
                <div class="modal-body request modal__request px-md-4 pb-md-4">
                    <div class="application__success" style="display: none;">
                        <div class="d-flex justify-content-center flex-column align-items-center py-5">
                            ...
                            <h5 class="text-center mt-4 w-65">{{__('messages.application.success')}}</h5>
                        </div>
                    </div>
                    <div class="application__error" style="display: none;">
                        <div class="d-flex justify-content-center flex-column align-items-center py-5">
                            ...
                            <h5 class="text-center mt-4 w-65">{{__('messages.application.error')}}</h5>
                        </div>
                    </div>
                    <form action="#" id="applicationPost" method="POST">
                        ...
                        <div class="form-group">
                            <label for="organization-rent">{{__('messages.application.organization')}} <span style="color: #E0001A">*</span> </label>
                            <input type="text" class="form-control" name="organization" id="organization-rent" placeholder="{{__('messages.application.organization')}}..." required>
                        </div>
                        <div class="form-row">
                            <label for="name-rent" class="col-12">{{__('messages.application.name')}} <span style="color: #E0001A">*</span></label>
                            <div class="form-group col-md-6">
                                <input type="text" class="form-control" name="name" id="name-rent" placeholder="{{__('messages.application.name.contact')}}..." required>
                            </div>
                            <div class="form-group col-md-6">
                                <input type="text" class="form-control" name="position" id="position-rent" placeholder="{{__('messages.application.name.position')}}..." required>
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="site" id="site-rent" placeholder="{{__('messages.application.site_or_insta')}}..." required>
                        </div>
                        <div class="form-group">
                            <label>{{__('messages.application.size')}} <span style="color: #E0001A">*</span></label>
                            <div class="form-group row">
                                <div class="col-sm-6 d-flex mb-3 mb-md-0 align-items-center">
                                    <small class="text-muted mr-2">{{__('messages.application.size_from')}}</small>
                                ...
                                <div class="col-sm-6 d-flex align-items-center">
                                    <small class="text-muted mr-2">{{__('messages.application.size_to')}}</small>
                        ...
                        <div class="navbar font-weight-normal p-0 mt-4" style="box-shadow: none">
                            <a class="navbar-brand color--accent font-weight-semibold" href="#">{{__('messages.additional')}}</a>
                            ...
                            <div class="collapse navbar-collapse" id="navbarSupportedContent1">
                                <div class="form-group">
                                    <label for="organization-rent">{{__('messages.application.product_type')}}<span style="color: #E0001A">*</span></label>
                                    <input type="text" class="form-control" name="product_type" id="product-type" placeholder="{{__('messages.application.product_type')}}..." required>
                                </div>
                                <div class="form-group">
                                    <label for="lifetime">{{__('messages.application.lifetime')}}<span style="color: #E0001A">*</span></label>
                                    <input type="text" class="form-control" name="lifetime" id="lifetime" placeholder="{{__('messages.application.lifetime')}}..." required>
                                </div>
                                <div class="form-group">
                                    <label for="vendor">{{__('messages.application.factory')}}<span style="color: #E0001A">*</span></label>
                                    <input type="text" class="form-control" name="factory" id="factory" placeholder="{{__('messages.application.factory')}}..." required>
                                </div>
                                <div class="form-group">
                                    <label for="you-are">{{__('messages.application.you_are')}}<span style="color: #E0001A">*</span></label>
                                    <select name="customer_type_id" id="you-are" class="mdb-select md-form" required>
                                        <option value="" disabled selected>{{__('messages.application.choose_from_record')}}</option>
                                        @foreach($customerTypes as $customerType)
                                            <option value="{{ $customerType->id }}">
                                                @if( app()->getLocale() == 'kk')
                                                    {{ $customerType->title_kz }}
                                                @else
                                                    {{ $customerType->title_ru    }}
                                                @endif
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    <label for="product-presented">{{__('messages.application.present_type')}}<span style="color: #E0001A">*</span></label>
                                    <select name="present_type" id="product-presented" class="mdb-select md-form" required>
                                        <option value="" disabled selected>{{__('messages.application.choose_from_record')}}</option>
                                        @foreach(['yes'=>__('messages.yes'),'no'=>__('messages.no'),'partially'=>__('messages.partially')] as $presentKey=>$presentValue)
                                            <option value="{{ $presentKey }}">{{ $presentValue }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="product-classification">{{__('messages.application.product_class')}}<span style="color: #E0001A">*</span></label>
                                    <select name="product_class" id="product-classification" class="mdb-select md-form" required>
                                        <option value="" disabled selected>{{__('messages.application.choose_from_record')}}</option>
                                        @foreach(['elite'=>__('messages.application.elite'),'middle'=>__('messages.application.middle'),'economy'=>__('messages.application.economy')] as $classKey=>$classValue)
                                            <option value="{{ $classKey }}">{{ $classValue }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="textarea-1">{{__('messages.application.assembly_work')}}</label>
                                    <textarea class="form-control" name="assembly_work" id="textarea-1" rows="5"></textarea>
                                </div>
                                <div class="form-group">
                                    <label for="textarea-2">{{__('messages.application.note')}}</label>
                        ...
                        <div class="text-center">
                            <button type="submit" class="button btn-primary rounded mt-3">{{__('messages.application.submit')}}</button>
                        </div>
                    </form>
                </div>
