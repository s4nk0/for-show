            ...
            <div class="subscribe__content">
                ...
                <p>{{__('messages.main.subscribe.first')}}
                    <span style="background: #FFF; color: #252525; display: inline-block; padding: 0 5px">
                        {{__('messages.main.subscribe.second')}}</span></p>
            </div>
            <div class="subscribe__right-side">
                <form id="subscribe" class="subscribe__form needs-validation" method="POST">
                    @csrf
                    <input type="email" name="email" placeholder="{{__('messages.main.subscribe.placeholder')}}" required>
                    <button type="submit" class="button button--accent">{{__('messages.main.subscribe.btn')}}</button>
                </form>
            </div>
        </div>
    </div>
