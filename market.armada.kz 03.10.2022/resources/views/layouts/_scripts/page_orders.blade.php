    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const editOrderWindow = (elem) => {
            if (elem.hasAttribute('data-type')) {
                ...
                if (elem.getAttribute('data-type') === 'additional-services') {
                    ...
                    item.textContent = '{{__('messages.me')}}';
        ...
        const requestToSubmitOrder = (storeId, storeElem, dopObj = {}) => {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data['answer'] === 'success') {
                        ...
                        for (let i = 0; i < card_info_arr.length; i++) {
                            localStorage.removeItem('ARMADA_PRODUCT_CART_' + card_info_arr[i].id + '_USER_' + authUserId);
                            localStorage.removeItem('ARMADA_PRODUCT_CART_' + card_info_arr[i].id + '_USER_' + 'null');
                ...
                error: function(data) {
                    showModal('{{__('messages.order.error.payment')}}');
                },
            });
        ...
        const checkToSubmitOrder = (target) => {
            if (target.tagName === 'BUTTON' && target.hasAttribute('data-storeid')) {
                if (document.getElementById('fio').value === '') {
                    showModal('{{__('messages.order.error.fio')}}');
                    return false;
                } else if (document.getElementById('phoneOrder').value === '') {
                    showModal('{{__('messages.order.error.tel')}}');
                    return false;
                } else if (document.getElementById('email').value === '') {
                    showModal('{{__('messages.order.error.email')}}');
                    return false;
                } else if (document.getElementById('city_plug').selected) {
                    showModal('{{__('messages.order.error.city')}}');
                    return false;
                } else if (document.getElementById('address').value === '') {
                    showModal('{{__('messages.order.error.address')}}');
                    return false;
                }
        ...
        const drawOrders = () => {
            if (document.location.pathname === '/orders' || document.location.pathname === '/kk/orders') {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (document.querySelector('#order-1') && document.querySelector('#order-1').hasAttribute('aria-modal') && document.querySelector('#other').checked &&
                ...
                client.textContent = '{{__('messages.me')}}';
