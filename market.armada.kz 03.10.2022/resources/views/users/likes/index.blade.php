    ...
    <div class="container page-user-area__content">
        <div class="row">
            ...
            <div class="page-user-area__favorites favorites col-12 col-lg-9">
                <div class="page-title">{{__('messages.profile.left_menu.likes')}}</div>
                        ...
                        <div class="favorites__header border-bottom border-top">
                            <form action="#" class="favorites__actions" method="POST" enctype="multipart/form-data">
                                ...
                                <button type="button" class="favorites__uncheck-all favorites__action cursor-pointer">
                                    ...
                                    <span>{{__('messages.cancel_all')}}</span>
                                </button>
                                <button type="button" class="favorites__delete favorites__action cursor-pointer">
                                    ...
                                    <span>{{__('messages.delete_select_all')}}</span>
                                </button>
                            </form>
                            <div class="favorites__sort">
                                <select class="browser-default custom-select" onchange="window.location.href=this.options[this.selectedIndex].value">
                                    <option value="{{ route('user.likes',['price'=>'up']) }}" @if(isset($_GET['price']) and $_GET['price'] == 'up') selected @endif>{{__('messages.by_price_asc')}}</option>
                                    <option value="{{ route('user.likes',['price'=>'down']) }}" @if(isset($_GET['price']) and $_GET['price'] == 'down') selected @endif>{{__('messages.by_price_desc')}}</option>
                                    <option value="{{ route('user.likes',['date'=>'down']) }}" @if(!isset($_GET['price']) or isset($_GET['date'])) selected @endif>{{__('messages.by_created_at')}}</option>
...
{{--                            @include('layouts._paginate',['pagination'=>$likes])--}}
                        ...
                        <div class="favorites__footer border-top d-flex flex-wrap align-items-center pt-3 mt-3 pt-md-4 mt-md-4">
                            <span class="mr-3 mr-md-4 mt-3">
                                <span class="favorites__count"><b>{{ $likesCount }}</b> {{__('messages.user.likes.products_with_count')}}</span>
                                <span class="favorites__total price__current">{{ number_format($likesSumPrice, 0, ',', ' ') }} <span class="currency">тг.</span></span>
                            </span>
                            <button data-likes="{{ $likes->toJson() }}" class="button btn-primary rounded mt-3" id="buyAllProducts" data-backdrop="false" data-toggle="modal" data-target="#addToBasket">{{__('messages.buy_all')}}</button>
                        </div>
                    @else
                        <p class="">{{__('messages.user.likes.empty')}}</p>
                    @endif
                </div>
            </div>
        </div>
