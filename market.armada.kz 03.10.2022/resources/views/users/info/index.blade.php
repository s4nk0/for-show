    ...
    <div class="container page-user-area__content">
        <div class="row">
            ...
            <div class="col-12 col-lg-9">
                <p class="page-title">{{__('messages.profile.title')}}</p>
...
{{--                @include('users.info._additional')--}}
                ...
                <button href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="button btn-outline-dark d-block d-sm-inline mt-5 mx-auto mx-sm-0">{{__('messages.auth.profile.logout')}}</button>
                <a href="{{ route('password.request') }}" class="button btn-outline-dark d-block d-sm-inline mt-5 mx-auto mx-sm-0">{{__('messages.auth.change_password')}}</a>
            </div>
        </div>
    </div>
@endsection
...
@push('scripts')
    ...
    <script>
        ...
        let hasPrefix = pattern.test(currentUrl);
        // hasPrefix ?'Сохранить' : 'Сақтау'
        {
                    ...
                    if( $( this ).hasClass('active') ) {
                        ...
                        if(applicationModalForm.valid()) {
                            $.ajax({
                                ...
                                beforeSend: function() {
                                    $editButton.find('span').text(!hasPrefix ?'Сохранение...' : 'Сақтау...');
                                    $editButton.css({
                                        'opacity':'.5'
                                    });
                                },
                                success: function(response) {
                                    ...
                                    $editButton.find('span').text(!hasPrefix ?'Редактировать...' : 'Түзету...');
                                            ...
                                            setTimeout(() => {
                                                let cloneInput = $( this )
                                                    ...
                                                    .attr({
                                                        ...
                                                        'placeholder':!hasPrefix ?'Текущий пароль' : 'Ағымдағы Құпия Сөз'//Ағымдағы Құпия Сөз
                                                    })
                                                    .insertBefore($( this ))
                                                    .fadeOut(200);
                        ...
                        $( this ).find('span').text(!hasPrefix ?'Сохранить' : 'Сақтау');
        ...
        $(document).ready(function() {
            const redirectCart = sessionStorage.getItem('redirect_cart_order');
