    ...
    <div class="user-block__wrapper d-flex">
        ...
        <form action="{{ route('user.contact') }}" class="user-block__wrap" method="POST">
            ...
            <div class="user-block__header">
                <div class="user-block__title">{{__('messages.main.contacts')}}</div>
            ...
            <div class="user-block__content">
                <ul class="user-block__items row">
                    <li class="user-block-info user-block__item col-12 col-md-6 col-xl-4">
                        <div class="user-block-info__name">{{__('messages.info_email')}}</div>
                    ...
                    @if(is_array($user->phones) and count($user->phones) > 0)
                        ...
                        @if(count($user->phones) < 2)
                            @for($i = count($user->phones) + 1;$i < 3; $i++)
                                <li class="user-block-info user-block__item col-12 col-md-6 col-xl-4">
                                    <div class="user-block-info__name">{{__('messages.info_tel')}} {{ $i }}</div>
                                    <div class="user-block-info__value">
                    ...
                    @else
                        @foreach([1,2] as $phone)
                            <li class="user-block-info user-block__item col-12 col-md-6 col-xl-4">
                                ...
                                <div class="user-block-info__value">
                                    <input type="tel" class="form-control" name="phones[]" placeholder="+xx (xxx) xxx-xx-xx" required>
                                    <span>{{__('messages.not_given')}}</span>
                                </div>
                            </li>
                        @endforeach
                    @endif
