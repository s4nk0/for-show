<aside class="filters filters--hidden col-12 col-md-3">
    ...
    <div class="filters__checkbox-group mt-3 mb-4">
        <p class="font-weight-semibold">{{__('messages.user.loyalty.filter.alfa')}}</p>
            ...
            <ul class="list-style-none mb-b p-0">
                @foreach([1,2,3,4,5,6,7,8] as $manufacturer)
                    <li class="custom-control @if($loop->first) filter-apply @endif custom-checkbox mb-2">
                        ...
                        <label for="vendor-{{ $loop->iteration }}" class="custom-control-label">{{__('messages.manufacturer')}} {{ $loop->iteration }}</label>
                    </li>
                @endforeach
            </ul>
        </div>
    ...
    <div class="filters__checkbox-group mt-4">
        <p class="font-weight-semibold">{{__('messages.country')}} <span style="text-transform: lowercase"></span>{{__('messages.manufacturer')}}</p>
        <div class="custom-scrollbar default-skin" style="max-height: 340px">
            <ul class="list-style-none mb-b p-0">
                @foreach([1,2,3,4,5,6] as $country)
                    <li class="custom-control custom-checkbox mb-2">
                        ...
                        <label for="country-{{ $loop->iteration }}" class="custom-control-label">{{__('messages.country')}} {{ $loop->iteration }}</label>
                    </li>
                @endforeach
            </ul>
        </div>
    </div>
    <button class="button btn-primary my-4 mr-4 float-right">{{__('messages.apply')}}</button>
</aside>
