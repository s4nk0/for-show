        ...
        <div class="row">
            ...
            <div class="orders col-12 col-lg-9">
                <div class="orders loyalty vendor__orders">
                    <h3 class="mb-3">{{__('messages.stores')}}</h3>
                                ...
                                <li class="loyalty__shop-wrap col-12 col-lg-6">
                                    <div class="loyalty-shop loyalty__shop d-flex rounded bg-light p-3">
                                        ...
                                        <div class="loyalty-shop__content ml-3">
                                            <div>
                                                <span>{{__('messages.user.order.seller')}}: </span><a href="{{ route('shop',$loyalties->where('store_id',$store->store_id)->first()->store->slug) }}" class="color--accent">{{ $loyalties->where('store_id',$store->store_id)->first()->store->title }}</a><br>
                                                <span>{{__('messages.bonuses')}}: </span><span class="color--accent">{{ $loyalties->where('store_id',$store->store_id)->sum('amt') }}</span><br>
                                                <div class="mb-2">{{__('messages.user.loyalty.send_bonus')}}:</div>
                                                <form action="#" class="d-flex align-items-center flex-wrap flex-xl-nowrap">
                                                    ...
                                                    <button type="submit" class="btn btn-md p-2 btn-primary shadow-none m-0 mb-2 font-weight-semibold" style="height: 38px; min-width:100px; box-shadow: none">{{__('messages.send')}}</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="orders__sort d-block d-md-none">
                        ...
                        <select class="browser-default custom-select" onchange="window.location.href=this.options[this.selectedIndex].value">
                            ...
                            <option value="{{ route('user.orders',['price'=>$price]) }}">{{__('messages.user.order.total')}}</option>
                            {{--                            <option value="8">! Товар</option>--}}
                        </select>
                    </div>
                    <div class="mb-4 d-flex align-items-center justify-content-between">
                        <h3>{{__('messages.actions')}}</h3>
                        <button class="d-none d-md-block button btn-light filters__open">{{__('messages.filter')}}</button>
                    </div>
                    <div class="orders__column-titles d-none d-xxl-flex align-items-center justify-content-between">
                        ...
                        <div class="orders__column orders__column--title">{{__('messages.date')}}</div>
                        <div class="orders__column orders__column--title">{{__('messages.user.order.seller')}}</div>
                        <div class="orders__column orders__column--title">{{__('messages.user.loyalty.points')}}</div>
                        <div class="orders__column orders__column--title">{{__('messages.comments')}}</div>
                        <div class="orders__column orders__column--title">{{__('messages.date_recorded')}}</div>
                    </div>
