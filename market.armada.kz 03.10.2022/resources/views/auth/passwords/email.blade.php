...
@section('content')
    ...
    <div class="page-login__bg" style="background: url('{{ asset('img/sections-bg/register-bg.jpg') }}')">
        ...
        <form class="flex-column justify-content-between h-100 needs-validation" novalidate method="POST" action="{{ route('password.email') }}">
            ...
            <div>
                <h3 class="h4 text-center login__title">{{ __('messages.auth.forgot')}}</h3>
                ...
                <div class="md-form md-outline input-with-pre-icon">
                    ...
                    <label for="forgot-login">{{ __('messages.auth.forgot.email') }}</label>
                    @if ($errors->has('email'))
                ...
                <button class="button btn-primary btn-block my-4 text-uppercase rounded" type="submit">{{__('messages.auth.forgot.change_password')}}</button>
                <div class="text-center">
                    <p>
                        <a href="{{ route('login') }}" class="font-weight-semibold text-underline ml-3 login__remembered-button">{{ __('messages.auth.forgot.remember') }}</a>
                    </p>
                </div>
            </div>
        </form>
