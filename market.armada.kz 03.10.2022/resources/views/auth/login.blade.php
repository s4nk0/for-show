...
@section('content')
    ...
    <div class="page-login__bg" style="background: url('{{ asset('img/sections-bg/register-bg.jpg') }}')">
        <section class="login page-login__login">
            <form @if(isset($_GET['default']) && $_GET['default'] == "1") action="{{ route('login') }}" class="d-flex flex-column justify-content-between h-100 default_form" @else action="{{ route('loginOTP') }}" class="d-flex flex-column justify-content-between h-100 otp_form" @endif  method="POST" >
            @if(isset($_GET['default']) && $_GET['default'] == "1")
                    ...
                    <div>
                        <h3 class="h4 text-center login__title">{{__('messages.auth')}}</h3>
                        ...
                        <div class="md-form md-outline input-with-pre-icon">
                            ...
                            <input type="hidden" value="{{$from_cart}}" name="from_cart">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <div>
                                <a href="{{ route('password.request') }}" class="btn-link text-underline">{{__('messages.auth.forgot')}}</a>
                            </div>
                        </div>
                        ...
                        <div class="text-center">
                            <p>{{__('messages.auth.no_account')}}
                                <a href="{{ route('register') }}" class="font-weight-semibold  ml-3">{{__('messages.auth.register')}}</a>
                            </p>
                        </div>
                    </div>
            ...
            @else
                ...
                @csrf
                <div class="md-form md-outline input-with-pre-icon">
                    <x-form.verify-phone-inputs label="{{__('messages.auth.register.tel')}}" type="tel" id="register-phone" value="{{ old('phone') }}" autocomplete="phone" placeholder="+7 (---) --- -- --" required />
                </div>

                <div class="text-center">
                    <p>{{__('messages.auth.not_available_phone')}}
                        <a href="/login?default=1" class="font-weight-semibold  ml-3 ">{{__('messages.auth.log_by')}}</a href="">
                    </p>
                </div>
            ...
            @endif
                <div class="text-center">
                    ...
                    {{--                            <span class="mr-2">{{__('messages.auth.log_as_seller')}}</span>--}}
                    ...
                    {{--                    </p>--}}
                    <p>
                        <span class="d-flex align-items-center justify-content-center font-weight-semibold">
                            <span class="mr-2">{!! __('messages.auth.sellerOrDesigner.register') !!}</span>
                            <svg width="8" height="11" viewBox="0 0 8 11" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M7.12779 5.10739L1.73144 0.162381C1.49466 -0.0541269 1.11102 -0.0541269 0.873636 0.162381C0.636848 0.378888 0.636848 0.730553 0.873636 0.947061L5.84195 5.4997L0.874234 10.0523C0.637446 10.2689 0.637446 10.6205 0.874234 10.8376C1.11102 11.0541 1.49525 11.0541 1.73204 10.8376L7.12839 5.89257C7.36159 5.6783 7.36159 5.32111 7.12779 5.10739Z" fill="#393939"/>
                            </svg>
                        </span>
                    ...
                    <div class="d-flex align-items-center justify-content-between" style="color: #787878;">
                        ...
                        <span class="col" style="white-space: nowrap">{{__('messages.auth.log_by')}}</span>
                        <span class="flex-grow-1"><hr></span>
                    </div>

                    <div class="d-flex justify-content-between mt-3">
                        <a href="{{route("facebookLogin")}}" class="social-login social-login--facebook flex-grow-1 mr-2 align-items-center">
                        ...
                        <a href="{{route('googleLogin')}}" class="social-login social-login--google flex-grow-1 ml-2 align-items-center">
                            <div class="social-login__icon">
...
@push('scripts')
    ...
    <script>
        $(document).ready(function (){
            $( ".to_default_form" ).click(function() {

                $('.default_form').css('display','block !important')
            });

            if ($('#login-password').val().length !== '' && $('#login-login').val().length !== ''){
