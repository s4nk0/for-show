<section class="reviews page-product__reviews position-relative">
    ...
    <div class="container">
        <div class="section__header reviews__header flex-column flex-md-row justify-content-center justify-content-md-between">
            <h2 class="section__title section__title--dark">{{__('messages.reviews')}} (<span>{{ $reviews->total() }}</span>)</h2>
            <div class="section__header-text d-block mt-3 mt-md-0">
                <button class="btn btn-outline-primary font-weight-semibold py-2 px-4" data-toggle="modal" data-target="#add-review">{{__('messages.product.leave_review')}}</button>
            </div>
        </div>
        ...
        {{--                        </div>--}}
            ...
            @else
                <br>
                <p>{{__('messages.product.first_to_leave_review')}}</p>
                <br>
            @endif
        </div>
        @if(isset($reviews) and $reviews->count() > 0)
