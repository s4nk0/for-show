...
@section('title', ( $product->title ?? 'Информация о продукте' ) . " - ARMADA"  )
@section('meta_description',( $product->title ?? 'ARMADA - мебельный торговый комплекс' ). " купить в ТК «ARMADA» ✅Большой выбор ✅ В рассрочку и без переплаты ✅ Доставка по всему Казахстану" )
...
@section('breadcrumb')
    @php
        ...
        $breadcrumbs[] = [  'route'=>route('subcatalog', $product->subcatalog->slug),
        ...
        <div class="modal-dialog modal-md" role="document">
            <form action="{{ route('productReview') }}" class="modal-content callback needs-validation" method="POST">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{__('messages.product.review')}}</h5>
                ...
                <div class="modal-body add-review reviews__add-review pb-0">
                    <div class="d-flex align-items-center mb-4">
                        <span>{{__('messages.product.review.choose')}}: </span>
                ...
                <div class="modal-footer p-3">
                    <button type="button" class="button btn-primary">{{__('messages.close')}}</button>
                    ...
                    <ul class="d-flex flex-column pl-0 mb-0">
                        ...
                        @isset($product->width)
                            <li class="d-flex mb-2">
                                ...
                                <span class="w-55">{{ $product->width ?? __('messages.missing') }} см</span>
                            </li>
                        @endisset
                        @isset($product->height)
                            <li class="d-flex mb-2">
                                <span class="w-35 text-muted">Ширина:</span>
                                <span class="w-55">{{ $product->height ?? __('messages.missing') }} см</span>
                            </li>
                        @endisset
                        @isset($product->depth)
                            <li class="d-flex mb-2">
                                <span class="w-35 text-muted">Глубина:</span>
                                <span class="w-55">{{ $product->depth ?? __('messages.missing') }} см</span>
                            </li>
                        @endisset
                        @isset($product->manufacture)
                            <li class="d-flex mb-2">
                                <span class="w-35 text-muted">Материалы:</span>
                                <span class="w-55">{{ $product->manufacture ?? __('messages.missing') }}</span>
                            </li>
                        @endisset
                        @isset($product->country_id)
                            <li class="d-flex mb-2">
                                ...
                                <span class="w-55">
                                     @if(app()->getLocale() == 'kk')
                                        {{ $product->country->title_kz ?? __('messages.missing') }}
                                    @else
                                        {{ $product->country->title_ru ?? __('messages.missing') }}
                                    @endif
                                </span>
                            </li>
                        @endisset
...
@push('scripts')
    ...
    <script src="{{ asset('js/dest/page-product/product.js') }}"></script>
    <script>
        ...
        $('.add_to_cart_show').find('span').text("{{__('messages.product.card.to_cart')}}")
        $('#mobile_add_to_cart_btn').click(function (){
            if($(this).hasClass("active") === false){
                $('#add_cart_dummy_text').hide()
                $('.btn_to_basket').css({"text-transform":"capitalize"})
            }
        })

        $(document).ready(function (){
            if($('#mobile_add_to_cart_btn').hasClass("active") === true){
                $('#add_cart_dummy_text').hide()
                $('.btn_to_basket').css({"text-transform":"capitalize"})
            }
        })
