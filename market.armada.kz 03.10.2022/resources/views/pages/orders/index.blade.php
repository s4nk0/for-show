...
@section('content')
    <h2 class="page-order__title container page-title pt-4">{{__('messages.order.title')}}</h2>
    ...
    <!--<form action="{{ route('orderPost') }}" class="order__form" method="POST">-->
        ...
        <div class="row">
            <div class="col-12 order__edit">
                <p class="h4 mb-4">{{__('messages.order.info_buyer')}}</p>

                <div class="row mb-4">
                    <div class="col-12 mb-3">
                        <label for="fio">{{__('messages.order.enter_fio')}} *</label>
                        <input type="text" name="fio" id="fio" class="form-control" @auth value="{{ old('fio') ?? Auth::user()->fio() }}" @endauth placeholder="{{__('messages.fio')}} *" required>
                    </div>
                    <div class="col-12 mb-3 mb-lg-0 col-md">
                        <label for="phone">{{__('messages.order.enter_tel')}}</label>
                        <input type="tel" name="phone" id="phoneOrder" class="form-control" @auth value="{{ old('phone') ?? Auth::user()->phone() }}" @endauth placeholder="{{__('messages.info_tel')}} *" required>
                    </div>
                    <div class="col-12 mb-3 mb-lg-0 col-md">
                        <label for="email">{{__('messages.order.enter_email')}}</label>
                        <input type="email" name="email" id="email" class="form-control" @auth value="{{ old('email') ?? Auth::user()->email }}" @endauth placeholder="{{__('messages.email')}} *" required>
                    </div>
                </div>

                <p class="h4 mb-4">{{__('messages.order.give_address')}}</p>

                <div class="row mb-4">
                    <div class="col-12 mb-3 col-md">
                        <label>{{__('messages.order.country_label')}}</label>
                        <p class="mt-2"><b>{{__('messages.kazakhstan')}}</b></p>
                    </div>
                    <div class="col-12 mb-3 col-md">
                        <label>{{__('messages.order.choose_city')}} *</label>
                        <select name="city_id" class="form-control mt-0" searchable="Поиск..." id="city" required>
                            <option id='city_plug' value="" disabled selected>{{__('messages.order.make_your_choice')}}</option>
                    ...
                    <div class="col-12 mb-3">
                        <label for="address">{{__('messages.order.delivery_address')}}</label>
                        <input type="text" name="address" id="address" class="form-control" placeholder="{{__('messages.order.delivery_address_placeholder')}}*" value="{{ old('address') ?? Auth::user()->address }}" required>
                    </div>
                    <div class="col-12">
                        <label for="comment">{{__('messages.order.comment')}}</label>
                        <textarea name="comment" class="form-control" id="comment" rows="6" placeholder="{{__('messages.order.comment_placeholder')}}"></textarea>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-12 col-lg-7">
                    <div class="order-item-total details__block bg-light rounded p-4 mt-4" data-payType="1,2,3,4{{-- collect($store->pay_id) --}}" data-deliveryType="1,2,3,4{{-- collect($store->delivery_id) --}}" data-dopserviceType="1,2,3,4{{-- collect($store->dopservice_id) --}}" data-storeId="{{ $store->id }}" data-order-index="1">
                        <h4 class="seller__title mb-4">{{__('messages.order')}} №{{ $loop->iteration }} ({{__('messages.store')}} <a href="{{ route('shop',$store->slug) }}" target="_blank">{{ $store->title }}</a>)</h4>
                        <ul class="seller__products row list-style-none mb-0 p-0">
                            @forelse($orders->where('store_id',$store->id) as $order)
                                <li class="col-12 seller__product order-total__item product-card product-card--vertical" data-productId="{{ $order['product']->id }}">
...
{{--                                                </span>--}}
                                        ...
                                        <a href="{{ route('product',['id' => $order['product']->id,'slug'=>$order['product']->slug]) }}" class="product-card__link" target="_blank">
                                            <div class="product-card__image">
                                                <img src="{{ '/storage/'.$order['image']}}" alt="Product image" />
                                            </div>
                                            <div class="product-card__content product-card__content--wp">
                                                ...
                                                <div class="price product-card__price">
                                                    @if($order['product']->is_discount === 1 and $order['product']->discount != null)
{{--                                                        <p></p>--}}
{{--                                                        <p>{{$order['product']->price}}</p>--}}
                                                        <strike class="price__previous">{{ number_format($order['product']->price, 0, ',', ' ') }} <span class="price__currency">тг.</span></strike>
                                                        <span class="price__special">{{ $order['product']->price*(1-$order['product']->discount/100) }} <span class="price__currency">тг</span></span>
                                                    @else
                                                        @if($order['product']->is_price_none != true)
                                                            @if($order['product']->is_price_from == true)
                                                                <span class="price__current">от {{ number_format($order['product']->price, 0, ',', ' ') }}
                                                                <span class="price__currency">тг.</span>
                                                            </span>
                                                            @else
                                                                <span class="price__current">{{ number_format($order['product']->price, 0, ',', ' ') }}
                                                                <span class="price__currency">тг.</span>
                                                            </span>
                                                            @endif
                                                        @else
                                                            <span class="price__current" style="font-size: 14px">Уточняйте у продавца</span>
                                                        @endIf
                                                    @endif
{{--                                                    @if($order['price'] !== '0')--}}
{{--                                                        <span class="price__current">{{ $order['price'] }} <span class="price__currency">тг.</span></span>--}}
{{--                                                    @else--}}
{{--                                                        <span class="price__current">{{__('messages.confirm_with_seller')}}</span>--}}
{{--                                                    @endif--}}


                                                </div>
                                                <div class="product-card__count">{{__('messages.count')}}: <span>{{ $order['count'] }}</span> шт.</div>
                                            </div>
                                        </a>
                                    </article>
                                </li>
                        ...
                        <p class="mt-4 d-flex align-items-center">
                            <span class="font-weight-bold text-uppercase mr-4">{{__('messages.order.order_details')}}</span>
                        ...
                        <ul class="order-item-total__details-items list-style-none pl-3">
                            <li class="order-item-total__details-item" data-seller-detail-type="delivery">
                                <p>
                                    <span>{{__('messages.order.delivery_method')}}:</span>
                                    <b class="d-inline-flex align-items-center">
                                        <span data-input-id-in-base="{{ $store->delivery_id != null ? $typeDeliverys->where('id',$store->delivery_id[0])->first()->id : 1 }}" class="information">{{ $store->delivery_id != null ? $typeDeliverys->where('id',$store->delivery_id[0])->first()->title : $typeDeliverys[0]->title ?? '' }}</span>
                                    </b>
                                </p>
                            </li>
                            <li class="order-item-total__details-item" data-seller-detail-type="payment">
                                <p>
                                    ...
                                    <b class="d-inline-flex align-items-center" >
                                        <span data-input-id-in-base="{{ $store->pay_id != null ? $typePays->where('id',$store->pay_id[0])->first()->id : 1 }}"
                                              class="information">
                                            {{ isset($store->pay_id) ? $typePays->where('id',$store->pay_id[0])->first()->title : $typePays[0]->title }}</span>
                                    </b>
                                </p>
                            </li>
                            <li class="order-item-total__details-item" data-seller-detail-type="recipient">
                                <p>
                                    <span>{{__('messages.order.receiver')}}:</span>
                                    <b class="d-inline-flex align-items-center">
                                        <span class="information">{{__('messages.me')}}</span>
                                    </b>
                                </p>
                            </li>
                ...
                <div class="order__details col-12 col-lg-5">
                    <div class="order__total bg-light rounded p-4 mt-lg-4 border-top">
                        <div class="seller__order-detail-total d-flex flex-column align-items-start justify-content-center">
                            <p class="h4 mb-4">{{__('messages.order.total')}} </p>
                            <div class="order-total__total mb-3">
                                <span>{{__('messages.order.for_pay')}}: </span>
...
{{--                            </div>--}}
                        {{--    <div class="custom-checkbox">
                            ...
                            <br>
                            <button data-storeid="{{ $store->id }}" type="submit" class="order-total__submit button btn-primary rounded">{{__('messages.order.confirm_order')}}</button>
                            <p class="medium mt-3 mb-0">{{__('messages.order.confirm_order_policy')}}</p>
                            <div class="seller__order-detail-promocode">
                                <p class="font-weight-bold text-uppercase mt-4">{{__('messages.order.add_promo_code')}}</p>
                                <p class="medium">{{__('messages.order.enter_promo_code')}}</p>
                                <div class="input-group mb-3 medium">
                                    <input type="text" name="promocode" class="form-control px-4" data-mask="0000-0000-0000-0000" placeholder="{{__('messages.order.promo_code')}}" aria-label="Введите промокод..." aria-describedby="button-addon2">
                                    <div class="input-group-append">
                                        <button class="btn btn-md btn-default m-0 px-4 font-weight-semibold py-2 z-depth-0 waves-effect text-uppercase text-white bg-dark py-3" type="button" id="button-addon2">{{__('messages.apply')}}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            sessionStorage.removeItem('redirect_cart_order');
        });
    </script>
@endpush
