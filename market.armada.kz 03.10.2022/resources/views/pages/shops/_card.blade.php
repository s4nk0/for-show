<li class="shop-card catalog__shop col-6 col-xl-4">
    <div class="shop-card__wrap">
        <div class="shop-card__image">
            ...
            <img src="{{ $item->getImages(0,'logo') }}"
                 class="shop-card__logo"
                 alt="{{ $item->title }}"
                 @if($item->getImages(0,'logo') === '/storage/')
                 style="display: none"
                @endif
            >
        ...
        <div class="shop-card__content">
            <div>
                ...
                @if(isset($item->block) or isset($item->intersection) or isset($item->butik))
                    ...
                    @for($i = 0; $i < max(count($item->block),count($item->intersection),count($item->butik)); $i++)
                        <div>
                            @isset($item->block[$i]){{__('messages.store.block')}} {{ $item->block[$i] }}, @endisset
                            @isset($item->intersection[$i]){{__('messages.store.intersection')}} {{ $item->intersection[$i] }}, @endisset
                            @isset($item->butik[$i]){{__('messages.store.butik')}} {{ $item->butik[$i] }} @endisset
                        </div>
                        @endfor
                        </p>
                        @endif
            ...
            <div class="shop-card__footer">
                @if ($item->is_Armada === 1)
                    ...
                    <span class="text-underline">{{__('messages.store.show_scheme')}}
                ...
                @endif
                <a href="{{ route('shop',['slug'=>$item->slug]) }}" class="shop-card__link button btn-outline-primary">
                    {{__('messages.store.in_store')}}
                </a>
            </div>
        </div>
    </div>
