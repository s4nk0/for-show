...
<li class="slideshow__item product-card col-6 col-lg-4 col-xxl-3 mb-4">
    ...
    <div class="card_store_location_popover card_store_location_popover-{{$item->id}}-{{$uniqueId}} @if($item->store->is_Armada) armada_location @else other_location @endif"
         data-id="location_popover_{{$item->id}}">
        <span><b>{{$item->store->title}}</b></span>
        <span>Адрес магазина:</span>
        <span>
            @if ($item->store->is_Armada)
                ТК "ARMADA"
                @if(isset($item->block) or isset($item->store->intersection) or isset($item->store->butik))
                    @for($i = 0; $i < max(count($item->store->block),count($item->store->intersection),count($item->store->butik)); $i++)
                        <div>
                            @isset($item->store->block[$i]){{__('messages.store.block')}} {{ $item->store->block[$i] }}, @endisset
                            @isset($item->store->intersection[$i]){{__('messages.store.intersection')}} {{ $item->store->intersection[$i] }}, @endisset
                            @isset($item->store->butik[$i]){{__('messages.store.butik')}} {{ $item->store->butik[$i] }} @endisset
                        </div>
                    @endfor
                @endif
            @else
                {!! $item->store->location ?? '<span class="text-danger font-weight-bold">Нет данных!!!</span>'!!}
            @endif
        </span>
    </div>
    <article class="product-card__wrap">
        <span class="product-card__header align-items-center">
            <span class="product-card__stickers">
                ...
                <span class="product-card__sticker product-card__sticker--sale">{{'-'.$item->discount.'%'}}</span>
                @endif
...
{{--{{ debug($item->id,$likeIds) }}--}}
            ...
            @if($item->store->is_Armada)
                <img src="{{ asset('img/svg/armada_logo.svg') }}" alt="" class="card_store_location_icon" style="width: 17px" width="23" height="23" data-id="{{$item->id}}-{{$uniqueId}}" id="card_store_location_icon-{{$item->id}}">
            @else
                <div class="card_store_location">
                    <img src="{{ asset('img/svg/card_location.svg') }}" class="card_store_location_icon" alt="card_location" data-id="{{$item->id}}-{{$uniqueId}}" id="card_store_location_icon-{{$item->id}}">
                </div>
        ...
        <a href="{{ route('product',['id'=>$item->id, 'slug' => $item->slug]) }}" class="product-card__link">
            <div class="product-card__image">
                <img class="lozad" data-src="{{ $item->getImages(0,'images', '/img/logo-gray.png',true) }}" alt="{{ ucfirst(strtolower($item->title)) }}" data-zoom-image="{{ $item->getImages() }}">
            </div>
...
{{--                    <p>{{$item->discount}}</p>--}}
                    @if($item->is_discount === 1 and $item->discount != null)
                        ...
                        <span class="price__special" style="margin-left: 0">{{ $item->price*(1-$item->discount/100) }} <span class="price__currency">тг</span></span>
                    @else
                        @if($item->is_price_none != true)
                            @if($item->is_price_from == true)
                                <span class="price__current">от {{ number_format($item->price, 0, ',', ' ') }}
                                <span class="price__currency">тг.</span>
                            </span>
                            @else
                            <span class="price__current">{{ number_format($item->price, 0, ',', ' ') }}
                                <span class="price__currency">тг.</span>
                            </span>
                            @endif
                        @else
...
{{--            To do this--}}
                ...
                <div class="product-card__add-to-card-button" data-backdrop="false" data-toggle="modal" data-target="#addToBasket">
                    <button class="btn_to_basket btn btn-primary ARMADA_PRODUCT_CART_{{ $item->id }}_CH" data-chosen = 'false' data-product-information='{{ $item->toJson() }}'>
                        <span class="btn_to_basket" data-product-information='{{ $item->toJson() }}'>{{__('messages.product.card.to_cart')}}</span>
