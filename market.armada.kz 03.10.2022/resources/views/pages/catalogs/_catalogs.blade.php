        ...
        <div class="catalog__content col-12 col-lg-9">
            <div class="catalog__header d-none d-lg-flex align-items-center justify-content-end">
                ...
                <select class="form-control w-auto bg-light border-0" id="select_filter" onchange="window.location.href=this.options[this.selectedIndex].value">
                    <option value="{{ route('subcatalog',['slug'=>$subcatalog->slug,'price'=>'down']) }}" @if(isset($_GET['price']) and $_GET['price'] == 'down') selected @endif>{{__('messages.by_price_desc')}}</option>
                    <option value="{{ route('subcatalog',['slug'=>$subcatalog->slug,'price'=>'up']) }}" @if(isset($_GET['price']) and $_GET['price'] == 'up') selected @endif>{{__('messages.by_price_asc')}}</option>
                    <option value="{{ route('subcatalog',['slug'=>$subcatalog->slug,'price'=>'discount']) }}" @if(isset($_GET['price']) and $_GET['price'] == 'discount') selected @endif>{{__('messages.by_discount')}}</option>
                    <option value="{{ route('subcatalog',['slug'=>$subcatalog->slug]) }}" @if(!isset($_GET['price'])) selected @endif>{{__('messages.without_sort')}}</option>
