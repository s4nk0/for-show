        ...
        <aside class="filters catalog__filters col-12 col-md-3">
            <form action="{{ route('item',['slug'=>$item->slug]) }}" method="GET">
                ...
                <div class="filters__checkbox-group py-4 mb-4">
                    <p class="font-weight-semibold">{{__('messages.manufacturer')}}</p>
                ...
                @if(!$colors->isEmpty())
                <div class="filters__checkbox-group py-4 mb-4">
                    <p class="font-weight-semibold">{{__('messages.color')}}</p>
                    <div class="custom-scrollbar default-skin" style="max-height: 340px">
                        <ul class="list-style-none mb-b p-0">
                            @foreach($colors as $color)
                                <li class="custom-control custom-checkbox mb-2"><!-- filter-apply  -->
                                    <input type="checkbox" name="color[]" value="{{ $color->slug }}" id="color-{{ $color->id }}" class="custom-control-input" @if(isset($_GET['color']) and in_array($color->slug,$_GET['color'])) checked @endif>
                                    <label for="color-{{ $color->id }}" class="custom-control-label">
                                        @if(app()->getLocale() == 'kk')
                                            {{ $color->title_kz }}
                                        @else
                                        {{ $color->title_ru }}
                ...
                <div class="filters__checkbox-group py-4 mb-4">
                    <p class="font-weight-semibold">{{__('messages.type_material')}}</p>
                    <div class="custom-scrollbar default-skin" style="max-height: 340px">
                        <ul class="list-style-none mb-b p-0">
                            @foreach($materials as $material)
                                <li class="custom-control custom-checkbox mb-2"><!-- filter-apply  -->
                                    <input type="checkbox" name="material[]" value="{{ $material }}" id="material-{{ $material }}" class="custom-control-input" @if(isset($_GET['material']) and in_array($material,$_GET['material'])) checked @endif>
                                    <label for="material-{{ $material }}" class="custom-control-label">{{ $material }}</label>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                        ...
                        <ul class="list-style-none mb-b p-0">
                            @foreach($countries as $country)
                                <li class="custom-control custom-checkbox mb-2">
                                    <input type="checkbox" name="country[]" value="{{ $country->id }}" id="country-{{ $country->id }}" class="custom-control-input" @if(isset($_GET['country']) and in_array($country->id,$_GET['country'])) checked @endif>
                                    <label for="country-{{ $country->id }}" class="custom-control-label">
                                        @if(app()->getLocale() == 'kk')
                                            {{ $country->title_kz }}
                                        @else
                                            {{ $country->title_ru }}
        ...
        <div class="catalog__content col-12 col-lg-9">
            <div class="catalog__header d-none d-lg-flex align-items-center justify-content-end">
                ...
                <select class="form-control w-auto bg-light border-0" id="select_filter" onchange="window.location.href=this.options[this.selectedIndex].value">
                    <option value="{{ route('item',['slug'=>$item->slug,'price'=>'down']) }}" @if(isset($_GET['price']) and $_GET['price'] == 'down') selected @endif>{{__('messages.by_price_desc')}}</option>
                    <option value="{{ route('item',['slug'=>$item->slug,'price'=>'up']) }}" @if(isset($_GET['price']) and $_GET['price'] == 'up') selected @endif>{{__('messages.by_price_asc')}}</option>
                    <option value="{{ route('item',['slug'=>$item->slug,'price'=>'discount']) }}" @if(isset($_GET['price']) and $_GET['price'] == 'discount') selected @endif>{{__('messages.by_discount')}}</option>
                    <option value="{{ route('item',['slug'=>$item->slug]) }}" @if(!isset($_GET['price'])) selected @endif>{{__('messages.without_sort')}}</option>
