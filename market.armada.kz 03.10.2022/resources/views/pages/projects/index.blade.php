...
@section('breadcrumb')
    @php
        ...
        $breadcrumbs[] = [  'route'=>route('projects'),
                            'title'=>__('messages.main.projects')  ];
    @endphp
                    ...
                    <div class="project__content-wrap col-12 col-xl-6">
                        <div class="project__content text-center">
                            ...
                            @if($project->phone != null)
                                <p>{{__('messages.project.edit_phone')}}: <a href="tel:{{ $project->phone }}">{{ $project->phone }}</a></p>
                            @endif
                            ...
                            @if($project->url != null)
                                <div class="project__button">
                                    <a href="{{ $project->url }}" target="_blank" class="button btn-outline-dark">{{__('messages.project.watch_channel_url')}}</a>
                                </div>
                            @endif
                        </div>
                    </div>
