                        ...
                        <div class="row">
                            ...
                            @foreach(config('translatable.locales') as $locale)
                                <div class="col-12 col-lg-5 mb-3">
                                    @include('layouts.forms.input',['name'=>$locale.'[title]','keyName'=>'title','locale'=>$locale,'label'=>'Вопрос('.strtoupper($locale).')','placeholder'=>'Вопрос','length'=>150,'required'=>true])
                                </div>
                            @endforeach
                        </div>
                            @foreach(config('translatable.locales') as $locale)
                                <div class="row">
                                    <div class="col-12 col-lg mb-3">
                                        @include('layouts.forms.textarea',['name'=>$locale.'[description]','key'=>'description','locale'=>$locale, 'class'=>'tinyMCE','label'=>'Ответ('.strtoupper($locale).')','placeholder'=>'Ответ('.strtoupper($locale).')','cols'=>30,'rows'=>15,'length'=>2000])
                                    </div>
                                </div>
                            @endforeach
