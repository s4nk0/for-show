@php
    ...
    $period = isset($_GET['period']) ? explode('-',$_GET['period']) : null;
    $start = $period != null ? explode('.',$period[0]) : null;
    $start_at = $start != null ? $start[0].'.'.$start[1].'.'.$start[2] : now()->addMonth(-1)->format('d.m.Y') ;
    $end = $period != null ? explode('.',$period[1]) : null;
    $end_at = $end != null ? $end[0].'.'.$end[1].'.'.$end[2] : now()->format('d.m.Y') ;
...
<script>
...

    {
        ...
        //
        //     let startDate = 'start=' + picker.startDate.format('DD.MM.YYYY');
        //     let endDate = 'end=' + picker.endDate.format('DD.MM.YYYY');
        //
        //     if(route.includes('?')) {
