...
{{--    <td>Мебельная фурнитура</td>--}}
    <td>
        ...
        {{ $order->product->item->subcatalog->title ?? "нет данных" }}<br>
        <span class="mx-2"><i class="fas fa-angle-double-right"></i></span>
        <span class="text-muted">{{ $order->product->item->subcatalog->catalog->title ?? "нет данных" }}</span>
    </td>
    <td>
    ...
    <td>
        <span class="font-weight-semibold products-table__price">
            @if(isset($order->price_2) && $order->price_2 > 0)
                {{ $order->price_2 }}
            @else
                {{ $order->price}}
            @endif
