...
{{--        <form action="POST" id="drag_drop_import" >--}}
{{--            <input type="file" name="dragged_files" id="fileInput_import" multiple />--}}
{{--        </form>--}}
...
{{--                                @endforeach--}}
                    ...
                    <form action="{{ route('admin.importdatapost') }}"  method="POST" enctype="multipart/form-data" class="mt-1">
                        @csrf
                        <div class="d-flex align-items-center">
                            <input type="hidden" name="import_store_id" id="import_store_id">
                            <div class="input-group d-flex align-items-center">
                                ...
                                <div class="custom-file cursor-pointer mt-2 mr-3" style="max-width: 320px; min-width: 250px;">
                                    <input type="file" name="file" class="custom-file-input" id="inputGroupFile01" oninvalid="this.setCustomValidity('Вы не добавили файл для загрузки'); handleFileImportRequired()" aria-describedby="inputGroupFileAddon01" required>
                                    <label class="custom-file-label" for="inputGroupFile01">Выберите файл</label>
                                </div>
                                <button type="submit" id="importdatapostBtn" class="buttons-copy buttons-html5 button btn-sm btn-light mt-2" style="font-size: 14px">Загрузить</button>
                                    <a href="{{ asset('files/armada_import.xlsx') }}" class=" mt-2 ml-3 text-underline" style="font-weight: 700;font-size: 14px">Шаблон</a>
                            </div>

                        </div>
                    </form>
                    <a href="{{ route('admin.export_data',['file_type'=>'csv','export_store_id'=> request()->get('store') ]) }}" type="submit" class="buttons-copy buttons-html5 button btn-sm btn-light rounded mr-2 mt-2 xmlExport d-none">CSV</a>
                    <a href="{{ route('admin.export_data',['file_type'=>'xlsx','export_store_id'=> request()->get('store')]) }}" type="submit" class="buttons-copy buttons-html5 button btn-sm btn-light rounded mr-2 mt-2 xmlExport d-none">XLSX</a>
                    <a href="{{ route('admin.export_data',['file_type'=>'xmlfile','export_store_id'=> request()->get('store')]) }}" type="submit" class="buttons-copy buttons-html5 button btn-sm btn-light rounded mr-2 mt-2 xmlExport d-none">XML</a>
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#import_store_id').val(store)

            $('#importdatapostBtn').click(function(){
                setTimeout(function (){
    ...
    <script>
        ...
        function handleFileImportRequired (){
...

        {
            ...
            function dropHandler(ev) {
                ...
                $.ajax({
                    ...
                    url: "{{route('admin.createWithImages')}}",
                    type: 'post',
                    data: formData,
