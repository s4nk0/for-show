            ...
            <div class="vendor__content flex-grow-1 vendor__content--products">
                ...
                <div class="orders vendor__shops pt-4 pb-3 p-xxl-3 pt-xxl-5 pr-0">
                    <div class="orders__header justify-content-between">
                        <h2 class="page-title orders__title">Изменить дизайнера</h2>
                    </div>
                    <form
                        id="admin_seller_update"
                        action="@isset($user){{ route('admin.designer.update',$user->id) }}@else{{ route('admin.designer.store') }}@endisset"
                        class="pb-4 border-bottom" method="POST" enctype="multipart/form-data">
                        @isset($user) @method('PATCH') @endisset
                        @csrf
                            <div class="col-12 col-lg-3">
                                @include('admin.designers._uploadImage', ['data'=>$user,'name'=>'avatar','limit'=>1])
                            </div>
                        <div class="row align-items-center">
                            ...
                            <div class="col-12">
                                ...
                                <div class="col-lg-6 mb-3">
                                    @include('layouts.forms.textarea',['name'=>'description','class'=>'tinyMCE','data'=>$user,'label'=>'Описание','placeholder'=>'Описание','cols'=>30,'rows'=>12,'length'=>2000,'required'=>true])
                                </div>
                                <div class="row">
                                    <div class="col-12 col-lg-6 mb-3">
                                        @include('layouts.forms.input',['name'=>'years','label'=>'Стаж','placeholder'=>'Стаж','maxlength'=>100,'data'=>$user])</div>
                                    <div class="col-6">
                                        ...
                                        <select name="styles[]" id="styles" class="form-control js-example-basic-multiple" multiple required>
                                            @foreach($styles as $style)
                                                <option  value="{{ $style->id }}" @if(isset($user->designerStyles) and is_array($user->designerStyles->pluck('id')->toArray()) and in_array($style->id,$user->designerStyles->pluck('id')->toArray())) selected @endif>
                                                    {{ $style->title }}
                                                </option>
                                            @endforeach
                                        </select>
                    ...
                    <div class="row">
                        ...
                        @else
                            <div class="col-12 col-lg-6 mb-3">
                                У данного дизайнера нет проектов
                            </div>
                        @endif
