...
class StoreControllerAdmin extends Controller
{
    ...
    {
        $tarifs = Tarif::all();
        $sellers =  User::where('role_id','=',User::SELLER)->select('id','name','email')->get();
        return view('admin.stores.credit', compact('tarifs','sellers'));
    }
    ...
    public function checkSellerExistsInStore(Request $request)
    {
        $store = Store::select('id','user_id')->with('user')->find($request->store_id);
        return response(isset($store->user_id) && $store->user_id != 0 && $store->user->role_id != User::ADMIN);
    }
    ...
    public function edit($id)
    {
        ...
        $sellers =  User::where('role_id','=',User::SELLER)->select('id','name','email')->get();
        return view('admin.stores.credit', compact('data', 'tarifs', 'products','sellers'));
    }
    ...
    public function update(StoreRequest $request, $id)
    {
        ...
        DB::transaction(function () use ($data, $request) {
            if (isset($request->block) && isset($data->block)) {
                if (count($request->block) > count($data->block)
                    && count($request->intersection) > count($data->intersection)
                    && count($request->butik) > count($data->butik)) {
                    $data->block = $request->block;
                    $data->intersection = $request->intersection;
                    $data->intersection = $request->butik;
                }
            }
