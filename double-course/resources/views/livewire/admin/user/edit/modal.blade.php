<form wire:submit.prevent="editUser" method="post" id="kt_account_profile_details_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" novalidate="novalidate">
@csrf
<!--begin::Card body-->
    <div class="card-body border-top p-9">
        ...
        <div class="row mb-6">
            ...
            <div class="col-lg-8 fv-row">
                ...
                <select wire:model="selected_roles" multiple aria-label="Select a Role" class="form-select form-select-solid form-select-lg fw-bold">
                    <option value="">Select a Role...</option>
                    @foreach($roles as $role)
                        <option value="{{$role->id}}" @if($user->hasRoles([$role->title])) selected="selected" checked @endif>{{$role->title}}</option>
                    @endforeach
                </select>
        ...
        <div class="row mb-6">
            ...
            <!--begin::Col-->
            <div class="col-lg-8 fv-row">
                <select wire:model="city" aria-label="Select a Country" data-placeholder="Select a country..." class="form-select form-select-solid form-select-lg fw-bold">
                    @foreach($cities as $city)
                        <option value="{{$city->id}}" @if($city->id == $user->city_id) selected="selected" @endif>{{$city->title}}</option>
                    @endforeach
                </select>
            </div>
            <!--end::Col-->
